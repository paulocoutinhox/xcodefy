...
A write is atomic everywhere, and safe to run concurrently on POSIX. A Windows `replace` cannot
serve simultaneous writers of one path, so the concurrency test is POSIX-only.
